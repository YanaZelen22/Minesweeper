...
public class GameService {
    ...
    private void open(Game game, int row, int col) {
        Queue<int[]> queue = new LinkedList<>();
        queue.add(new int[]{row, col}); // row=y, col=x

        while (!queue.isEmpty()) {
            int[] pos = queue.poll();
            int y = pos[0], x = pos[1];

            if (game.getOpen()[y][x]) continue;
            game.getOpen()[y][x] = true;

            if (game.getNumbers()[y][x] != 0) continue;

            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    int ny = y + dy, nx = x + dx;
                    if (nx < 0 || ny < 0 || nx >= game.getWidth() || ny >= game.getHeight()) continue;
                    if (!game.getOpen()[ny][nx]) queue.add(new int[]{ny, nx});
                }
            }
        }
    }
